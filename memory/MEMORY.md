...
- Key notes (verified against INFORMATION_SCHEMA + live counts 2026-09-18): `MACHINE_OPEN` is **TEXT** (`'true'`/NULL, no `'false'`) — human opens are `MACHINE_OPEN IS NULL`; never `IS NOT TRUE` (boolean syntax on a TEXT column). 89% of opens are machine opens. `IS_SUSPECTED_BOT_CLICK` is **BOOLEAN** (not string) and **100% NULL — Braze does not populate it, so any filter on it is a no-op and bot clicks are NOT being filtered**; use `= FALSE` not `= 'false'`

...
8. **Conversion event 1st/4th slots not getting selected — confirmed 2026-09-08.** This builder configures conversion events via `configure_conversions()` (imported from `build_pt_campaign.py`, shared with the older DnD builder's `configure_conversions_designed()` in `build_designed_campaign.py`). CZ's config (`data/brand_config.yaml`) has 3 of 4 conversion slots (A, C, D) as custom events — only B is built-in. The loop processes slots A→D and calls `_fill_custom_event_name()` for each non-built-in slot, but that function picked the target "Custom event name" field via `custom_labels.last` — the *last* such field anywhere on the page, not the one belonging to the slot currently being configured. On a seed-duplicated campaign (CZ's normal path — all 4 slots pre-exist from the ref campaign), this meant every custom-slot fill landed on the same (last-rendered, i.e. D's) field: A and C's own fields were never written at all (left showing whatever the seed/ref campaign happened to have), and D got overwritten repeatedly in the wrong order. Fixed by threading a `custom_rank` counter (0-based count of non-built-in slots processed so far, in A→D order) through `_select_conversion_event_type()` into `_fill_custom_event_name()`, which now selects `custom_labels.nth(custom_rank)` instead of `.last` — since Braze renders slots top-to-bottom in A-D order, the slot being configured is always the `custom_rank`-th "Custom event name" field on the page regardless of how many other slots already have a pre-filled custom name from the seed. Fixed in both `build_pt_campaign.py` and `build_designed_campaign.py` (shared bug), plus the one-off `scripts/fix_cz_sunwashed_isles_conversions.py` that had previously worked around this same symptom by hand for a single campaign.
...
GA4 session/traffic data is available via `scripts/snowflake_client.py` (used directly by most scripts/reports) and the `snowflake` MCP server. As of 2026-09-15, `scripts/snowflake_client.py` connects as a dedicated service account (`SVC_LIFECYCLE`, role `SVC_LIFECYCLE_RO`) using RSA key-pair auth (`SNOWFLAKE_PRIVATE_KEY_B64` in `.env`) — this replaced password auth, which broke when Snowflake enforced MFA on password sign-ins (2026-09-11). The `snowflake` MCP server (`.mcp.json`, a separate third-party binary) has not been updated and may still be on the dead password-auth path.
...
| `USERS_MESSAGES_EMAIL_OPEN_SHARED` | `MACHINE_OPEN` (**TEXT**, not bool — `'true'` / `NULL`), `USER_AGENT`, `IS_AMP` |
...
#
# Pre-aggregate each side in its own CTE, then join. Do NOT join the send and
# open views directly on DISPATCH_ID — it is a per-BATCH id (~81 users share
# one, verified 2026-09-18), so a direct join fans out ~81x. COUNT(DISTINCT)
# hides that just enough to look plausible while any SUM() is 81x too big.
results = client.execute_query(f"""
    WITH sends AS (
        SELECT CAMPAIGN_API_ID,
               TO_DATE(TO_TIMESTAMP(TIME)) AS send_date,
        ...
        WHERE APP_GROUP_ID = '67093a1f24ebbe0065cb9c77'  -- BUR
          -- integer epoch: prunes micro-partitions.
          -- TO_TIMESTAMP(TIME) >= ... converts every row and does not.
          AND TIME >= DATE_PART(epoch_second, DATEADD('day', -30, CURRENT_TIMESTAMP()))
        GROUP BY 1, 2
    ),
    opens AS (
        SELECT CAMPAIGN_API_ID,
               TO_DATE(TO_TIMESTAMP(TIME)) AS send_date,
               COUNT(DISTINCT USER_ID)     AS unique_openers
        FROM {DB}.{SCHEMA}.USERS_MESSAGES_EMAIL_OPEN_SHARED
        WHERE APP_GROUP_ID = '67093a1f24ebbe0065cb9c77'
          AND TIME >= DATE_PART(epoch_second, DATEADD('day', -30, CURRENT_TIMESTAMP()))
          -- MACHINE_OPEN is TEXT ('true' / NULL); human opens are NULL.
          -- Omit this line to match the Braze dashboard (which counts machine
          -- opens) — but be consistent, and say which convention you used.
          AND MACHINE_OPEN IS NULL
        GROUP BY 1, 2
    ),
    names AS (
        -- ~5.3 rows per campaign (one per save/rename). Without QUALIFY this
        -- multiplies result rows AND can return a stale "Copy of…" name.
        SELECT API_ID, NAME
    ...
    SELECT s.send_date,
           ...
           s.recipients,
           o.unique_openers,
           o.unique_openers / NULLIF(s.recipients, 0) AS unique_open_rate
    FROM sends s
    LEFT JOIN opens o USING (CAMPAIGN_API_ID, send_date)
    LEFT JOIN names n ON s.CAMPAIGN_API_ID = n.API_ID
    ORDER BY s.send_date DESC
""")
```

**Open rate is against unique RECIPIENTS, not sends.** A multi-step canvas sends
one person several emails, so a sends-based rate reads far below the Braze
dashboard and generates "the report is wrong" tickets.

### Important Notes
- **`MACHINE_OPEN` is `TEXT`, not boolean** (verified against `INFORMATION_SCHEMA` 2026-09-18). Values are `'true'` and `NULL` — **`'false'` never appears**. Human opens are `MACHINE_OPEN IS NULL`. Do **not** use `IS NOT TRUE`: that is boolean syntax against a TEXT column. The defensive form used across the repo, `(MACHINE_OPEN IS NULL OR MACHINE_OPEN = 'false')`, is correct — the second branch simply never matches. **89% of opens are machine opens** (539,536 of 607,638 over 2 days), so filtering vs. not filtering differs ~9x. The repo convention is to *include* them so numbers match the Braze dashboard — whichever you choose, be consistent and state it.
- **`IS_SUSPECTED_BOT_CLICK` is `BOOLEAN`, and was 100% `NULL`** (10,019 rows over 2 days, zero `TRUE`, zero `FALSE`, verified 2026-09-18). **Braze does not populate this column in this datashare, so any filter on it is a no-op — bot clicks are NOT being filtered out.** Keep `(IS_SUSPECTED_BOT_CLICK IS NULL OR IS_SUSPECTED_BOT_CLICK = FALSE)` for correctness, but do not treat it as bot protection. Note the repo's queries use `= 'false'` (string) — that works via Snowflake's implicit cast but is wrong-typed; prefer `FALSE`. Re-check with `warehouse-query-gotchas/verify.py` if Braze starts populating it.
- **`DISPATCH_ID` is per-BATCH, not per-user** — ~81 sends share one (verified 2026-09-18). **Never join the send and open views on `DISPATCH_ID` alone**; it fans out ~81x. Pre-aggregate each side in a CTE (see the example above), or join on `USER_ID` **and** `DISPATCH_ID` together for user-level rows. `USERS_MESSAGES_SMS_SHORTLINKCLICK_SHARED` has no `DISPATCH_ID` at all — join it on `CAMPAIGN_API_ID` / `CANVAS_API_ID`.
- `CAMPAIGN_API_ID` links events to campaign names via `CHANGELOGS_CAMPAIGN_SHARED` — always use a `QUALIFY ROW_NUMBER() OVER (PARTITION BY API_ID ORDER BY TIME DESC) = 1` subquery; a bare join returns multiple rows per campaign (one per save/rename) and will produce "Copy_…" names for campaigns that were duplicated then renamed. **Measured 2026-09-18: 26,713 rows / 4,998 campaigns = 5.3 rows each (worst case 31), and 3,773 campaigns — 75% — carry a "Copy of…" changelog row**, so the stale-name failure is the common case, not an edge case. `CHANGELOGS_CANVAS_SHARED` is 3.2 rows per canvas; same fix.

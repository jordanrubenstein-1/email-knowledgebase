...
def render_summary_banner(ty_email, ly_email, ty_sms, ly_sms, ty_ga4, ly_ga4,
    ...
    with metrics_col:
        cols = st.columns(5)
        with cols[0]:
            st.metric("Email Sends", num(ty_sends),
                      delta=f"{delta_pct(ty_sends, ly_sends):.1%} vs LY" if ly_sends else None)
        with cols[1]:
            st.metric("SMS Sends", num(ty_sms_sends),
                      delta=f"{delta_pct(ty_sms_sends, ly_sms_sends):.1%} vs LY" if ly_sms_sends else None)
        with cols[2]:
            st.metric("Sessions", num(ty_total_sess),
                      delta=f"{delta_pct(ty_total_sess, ly_total_sess):.1%} vs LY" if ly_total_sess else None)
        with cols[3]:
            st.metric("Triggered % of Revenue", pct(ty_trg_pct), delta=trg_delta)
        with cols[4]:
            st.metric("Lifecycle Revenue", money(ty_total_rev),
                      delta=f"{delta_pct(ty_total_rev, ly_total_rev):.1%} vs LY" if ly_total_rev else None)

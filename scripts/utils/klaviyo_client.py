...
class KlaviyoClient:
    ...
    def bulk_unsubscribe_profiles(
        ...
        """Globally unsubscribe `emails` from email marketing consent.

        Superseded 2026-09-23: this previously called the "Suppress" bulk job
        (profile-suppression-bulk-create-jobs), which turned out to be
        unreliable in practice -- one real profile sat at SUBSCRIBED for
        almost 24 hours after being submitted (well past the ~21-minute delay
        first observed and assumed to be the norm), and only actually flipped
        once unsubscribed via Klaviyo's own MCP unsubscribe tool instead.
        That tool's effect is immediate, and matches the documented Bulk
        Unsubscribe Profiles endpoint below -- so this now calls that
        endpoint directly rather than Suppress. Confirmed live 2026-09-23
        that no `list` relationship is required for a global (not list-
        scoped) unsubscribe, despite Klaviyo's own docs implying one is.

        This is NOT the profile-subscription-bulk-create-jobs endpoint used
        by bulk_subscribe_profiles_to_list -- that endpoint's `consent` field
        only accepts 'SUBSCRIBED' and 400s on 'UNSUBSCRIBED'. This uses the
        separate profile-subscription-bulk-delete-jobs endpoint, which does
        accept 'UNSUBSCRIBED'.

        Do NOT add profiles to a Klaviyo list -- confirmed 2026-09-22 that
        adding to a *single opt-in* list actively un-suppresses and subscribes
        the profile (the opposite of unsubscribing), and removing list
        membership afterward does not undo it.

        Returns count submitted. Confirmed the effect lands within seconds
        (unlike the old Suppress-based approach) -- safe to verify in the
        same run shortly after submitting."""
        submitted = 0
        for i in range(0, len(emails), chunk_size):
            chunk = emails[i:i + chunk_size]
            body = {
            ...
            if self._post_no_content("profile-subscription-bulk-delete-jobs/", body):
                submitted += len(chunk)
        return submitted

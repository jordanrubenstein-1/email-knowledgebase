...
def record_successful_run(dry_run: bool, skip: bool = False):
    """
...
def main():
    ...
    parser.add_argument("--emails", default=None,
                        help="Comma-separated addresses to process, skipping Snowflake detection "
                             "entirely. Escape hatch for when the datashare is unreachable (e.g. "
                             "Snowflake MFA lockout) and detection was run elsewhere. Safelist, "
                             "flagged-state and threshold checks still apply.")
    ...
    externally_supplied = bool(args.emails)

    if externally_supplied:
        supplied = [e.strip() for e in args.emails.split(",") if e.strip()]
        print(f"{prefix}Snowflake detection SKIPPED — {len(supplied)} address(es) supplied "
              f"via --emails.")
        for e in supplied:
            reasons.setdefault(e, set()).add("supplied")
    else:
    ...
    # get_latest_bounce_times() also hits the datashare, so it must be skipped on
    # the --emails path. Falling back to an empty map is safe in both directions:
    # nothing gets wrongly suppressed (an address with no known bounce time is
    # treated as new), and each address is then flagged at "now" below, which is
    # later than the bounce that surfaced it — so a genuinely newer bounce still
    # resurfaces on a later run.
    latest_bounce_times = {} if externally_supplied else get_latest_bounce_times(list(reasons))
    already_flagged = {}
    new_reasons = {}
    ...
    if not candidates:
        ...
        record_successful_run(args.dry_run, skip=externally_supplied)  # always advance the scan anchor
        return
    ...
    record_successful_run(args.dry_run, skip=externally_supplied)

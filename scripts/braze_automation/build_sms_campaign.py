...
def extract_sms_copy(
    ...
    if url_lines:
        ...
        #
        # Join ALL candidate lines here, not just `url_lines[0]`. When the copy
        # sentence and its link are written on separate lines (e.g. "Brand: 25%
        # off sitewide. Shop now:\nhttps://...") — the exact format used by the
        # SMS Brief Format in CLAUDE.md and by the self-generated brief pipeline
        # — the copy-only line has no URL and is excluded from `url_lines`, so
        # `raw_copy = url_lines[0]` silently dropped it and left only the bare
        # link as the SMS body. Confirmed 2026-09-08 on the CZ Best Sellers SMS
        # (P_SMS_2026_09_11_CZ_Best_Sellers), which built with no copy at all.
        # The other two branches below already join every candidate line for
        # exactly this reason — this branch had regressed to picking one line.

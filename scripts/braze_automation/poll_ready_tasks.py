...
# Shared consecutive-failure cap, also enforced by webhook_server.py. Lives in
# utils/ so the two build paths cannot drift apart. See that module for why.
from utils import build_retry_state
...
async def _poll_pt_email() -> None:
    ...
    for task in raw_tasks:
        ...
        # Send window has closed — building now can only produce a campaign
        # nobody will send. Tasks with no due_on fall through (unscheduled work).
        due_on = task.get("due_on")
        if due_on and due_on < date.today().isoformat():
            logger.debug(
                f"[PT Email] {gid} ({task.get('name', '')!r}) due {due_on} — due date passed, skipping"
            )
            continue

        # Stop retrying a task that keeps failing for the same reason. The
        # webhook path owns notifying the human; this path just stops picking
        # it up so the two don't double-comment. See utils/build_retry_state.py.
        if gid and build_retry_state.is_capped(gid):
            logger.debug(
                f"[PT Email] {gid} ({task.get('name', '')!r}) past retry cap "
                f"({build_retry_state.failure_count(gid)} failures) — skipping"
            )
            continue

#!/usr/bin/env python3
"""Edit the existing STF PT Labor Day Event Final Hours PM campaign in Braze.

Fixes two bugs in the auto-built draft:
  1. Subject Line field was wrong/blank — the auto-builder failed to recognize
     the bare first line of the Asana brief ("Final hours for 20% off", with
     no "SL:" label) as the subject line, so it never made it into the
     Subject field.
  2. The email body had the "Hi {{first_name}}," greeting duplicated, with
     "Final hours for 20% off" and a blank line sandwiched between the two
     occurrences — because the un-recognized subject line stayed stuck in the
     body ahead of the greeting, so the "strip leading greeting" step in the
     parser never fired.

Both bugs are already fixed in build_pt_campaign.py's _preprocess_body_copy()
(tertiary subject-line fallback, confirmed against this exact task). This
script regenerates subject + HTML from the Asana task using the now-fixed
parser, then edits the existing Braze draft in place (Edit Draft -> Compose ->
Edit message -> fill subject + HTML -> UTM templates -> Done -> Save as
draft). Does NOT send or schedule.
...
ASANA_GID = "1218024407555518"
SCRIPT_DIR = Path(__file__).parent


async def _debug(page, name: str) -> None:
    try:
        path = str(SCRIPT_DIR / f"debug_stf_pt_edit_{name}.png")
...
def main() -> None:
    ...
    # Sanity checks before touching Braze: correct subject, no stray subject
    # text in the body, exactly one greeting.
    expected_subject = "Final hours for 20% off"
    if subject != expected_subject:
        raise RuntimeError(f"Expected subject {expected_subject!r}, got {subject!r} — aborting.")
    if "Final hours for 20% off" in html_body:
        raise RuntimeError("Regenerated HTML still contains the stray 'Final hours for 20% off' line in the body — aborting.")
    if html_body.count("Hi {{${first_name}") != 1:
        raise RuntimeError(
            f"Expected exactly 1 greeting, found {html_body.count('Hi {{${first_name}')} — aborting."
        )
    logger.info("Sanity checks passed: subject correct, no stray subject text in body, exactly 1 greeting.")

...
#
# "ecommerce.order_placed" and "ecommerce.cart_updated" (CZ's A/D conversion
# slots, see data/brand_config.yaml) used to be plain custom events requiring
# "Performs Custom Event" + a typed event name. Braze has since split them out
# into their own direct dropdown options — "Places order" and "Update cart" —
# same as "Starts Session"/"Makes Purchase" below. Confirmed 2026-09-17: kept
# on the old custom-event path, these two would silently mis-select (either
# fail to find a "Performs Custom Event" candidate that no longer behaves the
# same way, or select the wrong option) on every CZ auto-built campaign.
BUILTIN_CONVERSION_EVENTS = {
    ...
    "ecommerce.order_placed": "Places order",
    "ecommerce.cart_updated": "Update cart",
}
...
async def configure_conversions(page: Page, conversions: Dict) -> bool:
    ...
    # custom_rank tracks how many slots processed so far (in A-D order) are
    # non-built-in — i.e. the 0-based position this slot's "Custom event name"
    # field will occupy among all "Custom event name" fields on the page, since
    # Braze renders slots top-to-bottom in A-D order. This replaces picking
    # "the last such field on the page", which always resolved to whichever
    # slot physically renders last (e.g. D) regardless of which slot was
    # actually being configured — see _fill_custom_event_name.
    custom_rank = 0
    for idx, slot in enumerate(["A", "B", "C", "D"]):
        ...
        await _select_conversion_event_type(page, idx, slot, is_builtin, braze_label, event_name, custom_rank)
        if not is_builtin:
            custom_rank += 1
...
async def _select_conversion_event_type(
    ...
    if not is_builtin:
        await _fill_custom_event_name(page, idx, slot, custom_event_name, custom_rank)

    return True


async def _fill_custom_event_name(
    page: Page, idx: int, slot: str, event_name: str, custom_rank: int = 0
) -> bool:
    ...
    ``custom_rank`` is the 0-based position of this slot's field among all
    "Custom event name" fields on the page, since Braze renders conversion
    slots top-to-bottom in A-D order — the slot currently being configured is
    always the (custom_rank)-th such field, regardless of how many other
    slots already have a custom event name pre-filled (e.g. from a
    duplicated/seed campaign). Previously this used ``.last``, which always
    resolved to whichever slot renders last on the page (typically D) no
    matter which slot was actually being configured — for a brand like CZ,
    where 3 of 4 slots (A, C, D) are custom events, every fill landed on the
    same field: A and C were silently never written, and D was repeatedly
    overwritten in the wrong order. Confirmed 2026-09-08 on CZ auto-built
    campaigns where the 1st and 4th conversion events weren't ending up
    correctly selected.
    """
    ...
    if custom_rank >= custom_label_count:
        logger.warning(
            f"Conversion {slot}: custom_rank {custom_rank} out of range "
            f"({custom_label_count} 'Custom event name' fields found) — falling back to last"
        )
        target_label = custom_labels.last
    else:
        target_label = custom_labels.nth(custom_rank)
    await target_label.scroll_into_view_if_needed()

...
def _clean_segment_name(name: Optional[str]) -> str:
    """Strip lifecycle_guidelines.yaml's trailing '*' footnote marker.

    Several entries append a documentation footnote asterisk to a segment
    name (e.g. "AM VIP Send List*", "AM List VIP*", "Geo_All*") that is never
    part of the actual Braze segment/filter name shown on the Target
    Audiences tab. Left in place, it makes an otherwise-correct name
    unmatchable via substring search — confirmed 2026-09-17 on CZ's
    "AM VIP Send List*" (the live campaign shows the filter as plain
    "AM VIP Send List", no asterisk).
    """
    return re.sub(r"\*+\s*$", "", (name or "").strip()).strip()


# Brands whose "Engaged" tier in Braze is actually built as the full-file
# base segment(s) PLUS an additional "member of segment" filter — not a
# separate standalone list. Confirmed for CZ 2026-09-17 (base "Full File"
# List segment + "AM VIP Send List" filter under Additional Filters, on
# P_EM_2026_09_22_CZ_D_Wall_Art_Mirrors) and already modeled for BUR via the
# "\nOR" branch in get_expected_segment_groups(). Do NOT add a brand here on
# a guess — e.g. STF's "engaged" name in lifecycle_guidelines.yaml is itself
# one real (if oddly-named) segment, not a base+filter combo — confirm
# against a live campaign's Target Audiences tab first.
_FILTER_BASED_ENGAGED_BRANDS = {"CZ", "BUR"}


def _resolve_standard_segment_tier(task: Optional[dict]) -> Optional[str]:
    """Read the Asana 'Segment' enum to learn which tier a task actually wants.

    Only meaningful for brands that pick Full File vs Engaged per task via
    the plain Segment enum field (i.e. everything except the Segment-(Text)
    brands ID/TI, which have their own resolver in
    _resolve_expected_audience_key()). Returns "full_file", "engaged", or
    None when the field is blank or names something this function doesn't
    model (e.g. Geo) — callers should fall back to permissive OR-across-all
    matching in that case, same as before this resolver existed.
    """
    if not task:
        return None
    raw = (_get_enum_value_name(task, FIELD_SEGMENT) or "").strip().lower()
    if "engaged" in raw:
        return "engaged"
    if "full file" in raw:
        return "full_file"
...
def get_expected_segment_groups(
    ...
    try:
        ...
        # Email: use all_subscribers + engaged.
        #
        # requested_tier tells us, from the task's own Segment enum, whether
        # this specific send actually wants Full File or Engaged — so a brand
        # in _FILTER_BASED_ENGAGED_BRANDS can be checked against the SPECIFIC
        # required configuration instead of "any of the possible ones",
        # which is what let a Full-File-only build silently pass QA as
        # "Engaged" before (the base segment is present either way; only the
        # additional filter actually distinguishes the two tiers).
        requested_tier = _resolve_standard_segment_tier(task)
        ...
        for brand_entry in matching:
            ...
            # must ALL be present in Additional Filters.
            #   full_file_group = the 4 filter segments
            #   engaged_group   = the 4 filter segments + the engaged filter segment
            if all_subs_raw and "\nOR" in all_subs_raw:
                filter_segs = [
                    _clean_segment_name(_BRAZE_SEGMENT_OVERRIDES.get(n, n))
                    for n in re.findall(r'"([^"]+)"', all_subs_raw)
                ]
                if filter_segs:
                    full_file_group = filter_segs
                    engaged_group = None
                    if engaged_raw and engaged_raw.strip().upper() not in ("N/A", "TBD", ""):
                        eng_resolved = _clean_segment_name(
                            _BRAZE_SEGMENT_OVERRIDES.get(engaged_raw, engaged_raw)
                        )
                        engaged_group = filter_segs + [eng_resolved]

                    if requested_tier == "engaged" and engaged_group:
                        groups.append(engaged_group)
                    elif requested_tier == "full_file":
                        groups.append(full_file_group)
                    else:
                        # Task's tier is unresolved (blank/Geo/unrecognized) —
                        # fall back to permissive OR-across-both, same as
                        # before requested_tier existed.
                        groups.append(full_file_group)
                        if engaged_group:
                            groups.append(engaged_group)
            ...
            all_subs_clean = (
                _clean_segment_name(_BRAZE_SEGMENT_OVERRIDES.get(all_subs_raw, all_subs_raw))
                if all_subs_raw and all_subs_raw.strip().upper() not in ("N/A", "TBD", "")
                else None
            )
            engaged_clean = (
                _clean_segment_name(_BRAZE_SEGMENT_OVERRIDES.get(engaged_raw, engaged_raw))
                if engaged_raw and engaged_raw.strip().upper() not in ("N/A", "TBD", "")
                else None
            )

            # Brands where "Engaged" is really "Full File base segment +
            # additional filter for the engaged segment" in Braze, not a
            # standalone list — require BOTH names together for that tier
            # instead of accepting the base segment alone.
            if brand in _FILTER_BASED_ENGAGED_BRANDS and all_subs_clean and engaged_clean:
                full_file_group = [all_subs_clean]
                engaged_group = [all_subs_clean, engaged_clean]
                if requested_tier == "engaged":
                    if engaged_group not in groups:
                        groups.append(engaged_group)
                elif requested_tier == "full_file":
                    if full_file_group not in groups:
                        groups.append(full_file_group)
                else:
                    for g in (full_file_group, engaged_group):
                        if g not in groups:
                            groups.append(g)
                continue

            # Standard brands: each segment is its own single-element group.
            for raw_clean in (all_subs_clean, engaged_clean):
                if not raw_clean:
                    continue
                candidate = [raw_clean]
                if candidate not in groups:
...
def _get_expected_delivery(
    ...
    Returns a dict with:
      use_sto (bool): best-guess prediction of whether Intelligent Timing
        should have been configured, based on lead time. This is a hint for
        logging only — verify_delivery() checks the actual Delivery tab and
        accepts either mode (fixed time, or STO with a matching fallback), so
        an imperfect prediction here does not by itself fail the QA check.
      time_str (str|None): expected time — the fixed send time if not on STO,
        or the STO fallback time if it is (e.g. "7:15 AM"). Either way this is
        the value verify_delivery() looks for on the page.
    ...
    builder never saw. Falls back to today when unknown — which is exactly the
    case where the ``use_sto`` prediction above is most likely to disagree with
    what was actually built (e.g. QA running close to send date on a campaign
    that was correctly built as STO days earlier, when there was still enough
    lead time); this is why verify_delivery() treats it as a hint, not a gate.
    """
...
def _extract_push_deep_link(msg: dict) -> str:
    ...
    """
    # "action" is the real field name Braze's campaigns/details API returns for
    # a push message's on-click behavior URL (confirmed 2026-09-11 against a
    # live campaign — e.g. {"action": "https://havenly.com/shop?utm_source=..."}).
    # None of "uri"/"url"/"deep_link"/"redirect_url" ever appear on a real push
    # message, so without this key this function always returned "" for a
    # correctly-configured push, and the three QA checks that depend on it
    # (on-click behavior set, link pre-approved, UTMs present) all failed even
    # though the campaign was built correctly.
    for key in ("action", "uri", "url", "deep_link", "redirect_url"):
        val = msg.get(key)
        if val:
            return val
...
# STO needs this many full calendar days between "today" (whenever QA happens
# to run) and the send date to have a real rolling delivery window to work
# with — this is a hard runway check on the CURRENT date, independent of
# whatever lead time existed back when the campaign was built. E.g. checked
# Monday: a Wednesday send has only 1 full day (Tuesday) between now and send
# — below this threshold, so STO should be flagged even if it was reasonably
# built as STO days earlier. A Thursday send has 2 full days (Tue + Wed) —
# fine.
STO_MIN_RUNWAY_FULL_DAYS = 2


def _sto_runway_full_days(due_on: str, today: Optional[date] = None) -> Optional[int]:
    """Full calendar days strictly between *today* and the due date.

    Returns None if due_on is missing/unparseable. Clamped to 0 for a due date
    that is today or in the past (no runway at all).
    """
    if not due_on:
        return None
    try:
        due_date = date.fromisoformat(due_on[:10])
    except (ValueError, TypeError):
        return None
    start = today or date.today()
    return max((due_date - start).days - 1, 0)


async def _read_delivery_time_input(page) -> Optional[str]:
    """Read the visible time-of-day text input's current value on the Delivery tab.

    Braze renders both the fixed "Send at a designated time" field and the
    Intelligent Timing custom-fallback-time field as a plain text <input>
    (placeholder "h:mm am") whose typed value lives in the DOM `value`
    attribute, not as rendered text content. `page.inner_text()` can never
    see it — confirmed 2026-09-17 on P_EM_2026_09_22_CZ_D_Wall_Art_Mirrors,
    which had "7:15 am" correctly set as the Intelligent Timing fallback (the
    input's own `value` attribute) but the Delivery check still reported the
    time as "not found" because it only ever scanned rendered page text.
    """
    try:
        candidates = page.locator('input[placeholder="h:mm am" i]')
        ...
        logger.warning(f"Could not read Delivery time input value: {exc}")
    return None


def _times_match(expected: str, actual: str) -> bool:
    """Compare two clock-time strings, tolerant of case, spacing, and a
    leading zero on the hour (Braze's input shows "7:15 am", expected
    strings are typically written as "7:15 AM")."""
...
async def verify_delivery(page, expected: dict) -> tuple[bool, bool]:
    ...
    Args:
        ...
        expected: dict from _get_expected_delivery() with keys:
            use_sto (bool): True if Intelligent Timing should be configured.
                This is only a *prediction* of which mode the builder should
                have used, based on lead time computed at build time — QA can
                run well after the build (or re-run days later), when the same
                lead-time math computed from "today" can give a different
                answer than it did when the campaign was actually built. It is
                logged for context but is NOT used to gate pass/fail below.
            time_str (str|None): expected send time — either the fixed send
                time, or the Intelligent Timing fallback time, whichever mode
                is actually configured (e.g. "7:15 AM").

    A campaign is considered correctly scheduled if EITHER:
      - Intelligent Timing (STO) is on and its fallback time matches time_str, or
      - it's a fixed-time send at time_str.
    Both are legitimate outcomes of the same build logic depending on how much
    lead time existed when the campaign was actually built, so neither is
    treated as "the mistake" — only a time_str that matches neither mode is.

    Returns (ok, has_sto) — ``has_sto`` reports what was actually found on the
    page (regardless of ``ok``) so the caller can separately check STO runway
    against today's date via ``_sto_runway_full_days()``.
    """
    use_sto: bool = expected.get("use_sto", False)
    time_str: Optional[str] = expected.get("time_str")

    logger.info(
        f"Checking Delivery tab (predicted use_sto={use_sto}, expected_time={time_str!r})..."
    )
    try:
        ...
        # Determine which delivery mode is actually selected by reading the
        # checked radio input directly (name="send-time-option", values
        # "immediate" / "scheduled" / "optimal" for Intelligent Timing) rather
        # than testing whether "Intelligent Timing" appears anywhere in the
        # page text. Braze always renders all three scheduling-mode labels
        # ("Send as soon as Campaign is launched" / "Send at a designated
        # time" / "Intelligent Timing") on the page regardless of which one is
        # selected, so the old substring check was a false positive on every
        # fixed-time send — confirmed 2026-09-15 on a St. Frank campaign
        # (P_EM_2026_09_16_SF_D_Destination_Tuscany, Asana task
        # 1217606435229387) that was correctly built with "Send at a
        # designated time" selected but still got flagged as STO.
        try:
            checked_value = await page.locator(
                'input[name="send-time-option"]:checked'
            ).get_attribute("value", timeout=3000)
        except Exception:
            checked_value = None
        if checked_value is not None:
            has_sto = checked_value == "optimal"
        else:
            # Fallback for an unexpected DOM shape — better a possible false
            # positive than silently skipping the check entirely.
            has_sto = "Intelligent Timing" in page_text

        time_found = False
        if time_str:
            candidates = [time_str, time_str.replace(" ", "")]
            time_found = any(c in page_text for c in candidates)
            if not time_found:
                # The time-of-day value lives inside a text <input>'s `value`
                # attribute, not rendered page text — read it directly rather
                # than relying on the substring scan above (see
                # _read_delivery_time_input()).
                input_val = await _read_delivery_time_input(page)
                if input_val and _times_match(time_str, input_val):
                    time_found = True
                    logger.info(
                        f"✓ Send time confirmed via input field value: {input_val!r}"
                    )

        if has_sto:
            # STO is on — pass if there's no specific time to check, or the
            # fallback time on the page matches what was expected.
            if not time_str or time_found:
                logger.info(
                    "✓ Intelligent Timing (STO) confirmed on Delivery tab"
                    + (f" with fallback {time_str!r}" if time_str else "")
                )
                return True, has_sto
            lines = [
                l.strip()
            ...
            logger.warning(
                f"Intelligent Timing (STO) confirmed but fallback time {time_str!r} "
                f"not found. Time-related lines: {lines[:5]}"
            )
            return False, has_sto
        else:
            # No Intelligent Timing on the page — this is a fixed-time send.
            if time_str:
                if time_found:
                    logger.info(f"✓ Send time confirmed: {time_str!r}")
                    return True, has_sto
                lines = [
                    l.strip()
                    for l in page_text.splitlines()
                ...
                logger.warning(
                    ...
                    f"(Intelligent Timing also not detected). Time-related lines: {lines[:5]}"
                )
                return False, has_sto
            else:
...
def _segment_name_on_page(name: str, page_text: str, known_names: set[str]) -> bool:
    ...
    # Quotes are stripped from the page line before comparing (Braze renders
    # some segment names with literal quotes, e.g. CZ/STF's
    # '"Full File" List - September 2024' via _BRAZE_SEGMENT_OVERRIDES), so
    # `name` itself must go through the same stripping — comparing a
    # quote-stripped page line against a quote-containing `name` can never
    # match. Confirmed 2026-09-08: this asymmetry (introduced alongside the
    # longest-match-wins fix below, which does strip quotes from `k`) made
    # verify_audience() report a false "did not match expected" for every
    # correctly-built CZ campaign.
...
async def run_qa_and_test_send(
    ...
    async with async_playwright() as p:
        ...
        send_time_ok, delivery_has_sto = await verify_delivery(page, delivery_expected)

        # STO runway check — independent of send_time_ok above. A campaign can
        # correctly have STO configured with the right fallback time and still
        # be a mistake right now if too little runway remains before send: STO
        # needs a real rolling delivery window, and a campaign QA'd this close
        # to its send date should be switched to a specific time instead.
        sto_runway_ok = True
        if delivery_has_sto:
            due_on = (raw_task or {}).get("due_on") or ""
            full_days = _sto_runway_full_days(due_on)
            if full_days is not None and full_days < STO_MIN_RUNWAY_FULL_DAYS:
                sto_runway_ok = False
                logger.warning(
                    f"Intelligent Timing (STO) is on but only {full_days} full day(s) remain "
                    f"before send (due_on={due_on!r}) — needs a specific send time instead"
                )
    ...
    if not send_time_ok:
        ...
        # verify_delivery() accepts either a fixed-time send or Intelligent
        # Timing (STO) with a matching fallback — both are legitimate, so a
        # failure here means the expected time wasn't found in *either* mode,
        # not that the wrong mode was used.
        t = delivery_expected.get("time_str") or "expected time"
        qa_issues.append(
            f"Send time ({t}) could not be confirmed in Braze, as either a fixed send time "
            "or an Intelligent Timing (STO) fallback — verify Delivery tab"
        )
    if not sto_runway_ok:
        due_on = (raw_task or {}).get("due_on") or ""
        full_days = _sto_runway_full_days(due_on)
        qa_issues.append(
            f"Scheduled using Intelligent Timing (STO), but only {full_days} full day(s) remain "
            f"before send (due {due_on}) — needs at least {STO_MIN_RUNWAY_FULL_DAYS} full days "
            "of runway to work correctly; change to a specific send time instead"
        )
    if not segment_ok:
        qa_issues.append(

...
from utils.sms_grammar import sms_copy_matches, sms_body_length_for_limit_check
...
def run_klaviyo_qa(
    ...
    if channel == "sms":
        ...
        # Substitute a LINK placeholder with the actual built URL first, same
        # as the Braze SMS QA path, so a briefed "LINK" token doesn't cause a
        # false mismatch once the real URL replaces it in the built body.
        url_in_actual = _extract_sms_link(sms_body) or ""
        expected_for_compare = (
            re.sub(r'\bLINK\b', url_in_actual, expected_copy) if url_in_actual else expected_copy
        )
        copy_ok = sms_copy_matches(expected_for_compare, sms_body)
        _check(
    ...
    # ------------------------------------------------------------------
    if channel == "sms":
        # Klaviyo (like Braze) shortens links at send time — count the link at
        # its shortened length, not the raw UTM-laden URL stored in content.
        effective_len = sms_body_length_for_limit_check(sms_body)
        char_ok = effective_len <= 130

...
# Build retry cap
#
# The "already built" guards in _check_and_build only recognise builds this
# service performed itself. A campaign a human built and launched by hand leaves
# neither marker, so if its Asana task is also left at Ready to Code, the
# Ready-to-Code poller re-queues it every ~35 minutes indefinitely.
#
# Implemented in scripts/utils/build_retry_state.py so this path and the
# 15-minute LaunchAgent poller (poll_ready_tasks.py) share one cap and one state
# file — they build through separate code paths, and this repo has been bitten
# before by routing that had to stay in lockstep across those two files.
# ---------------------------------------------------------------------------
from utils.build_retry_state import (  # noqa: E402
    BUILD_RETRY_CAP,
    clear as _clear_build_failures,
    failure_count as _build_failure_count,
    get_entry as _build_retry_entry,
    mark_capped_notified as _mark_capped_notified,
    record_failure as _record_build_failure,
    remember_task_name as _remember_task_name,
)
...
def _fetch_ready_to_code_tasks() -> list[dict]:
    ...
    Filters to due_on >= today at the API level so the 100-task limit only
    ...
    """
    # Tightened from yesterday to today on 2026-09-13: a task whose send window
    # has already closed must never be re-queued. See BUILD_RETRY_CAP.
    ...
    resp = _asana_request(
        ...
        params={
            ...
            "projects.any": MASTER_PROJECT_GID,
            "due_on.after": cutoff,
            "opt_fields": "gid,name,due_on,completed,custom_fields.gid,custom_fields.enum_value",
            "limit": "100",
        },
    )
    return resp or []
        ...
        try:
            ...
            for task in tasks:
                ...
                if task.get("completed"):
                    logger.debug(f"[Poll-RTC] {gid} ({name!r}) completed — skipping")
                    continue
                due_on = task.get("due_on")
                if due_on and due_on < date.today().isoformat():
                    logger.debug(f"[Poll-RTC] {gid} ({name!r}) due {due_on} — due date passed, skipping")
                    continue
...
async def _dispatch_single_slice_designed_build(
    ...
    if result.get("success"):
        ...
        _record_build_failure(task_gid, "", error_msg)
...
async def _check_and_build(task_gid: str) -> None:
    ...
    task_name_for_guards = raw_task.get("name", "")
    _remember_task_name(task_gid, task_name_for_guards)

    # --- Check: task is completed ---
    # A completed task is done regardless of what its Task Status still reads.
    if raw_task.get("completed"):
        logger.info(f"Task {task_gid} ({task_name_for_guards!r}) is completed — skipping")
        _clear_build_failures(task_gid)
        return

    # --- Check: due date has passed ---
    # The send window is over; building now can only produce a campaign nobody
    # will send. Tasks with no due_on are allowed through (webhook-driven builds
    # for unscheduled work).
    due_on_guard = raw_task.get("due_on")
    if due_on_guard and due_on_guard < date.today().isoformat():
        logger.info(
            f"Task {task_gid} ({task_name_for_guards!r}) due {due_on_guard} — "
            f"due date passed, skipping"
        )
        _clear_build_failures(task_gid)
        return

    # --- Check: consecutive-failure cap ---
    # Stop re-queueing a task that keeps failing for the same reason, and tell a
    # human once. See BUILD_RETRY_CAP above for why this exists.
    failures = _build_failure_count(task_gid)
    if failures >= BUILD_RETRY_CAP:
        entry = _build_retry_entry(task_gid)
        if not entry.get("capped_notified"):
            ...
            await asyncio.to_thread(
                ...
                f"tasks/{task_gid}/stories",
                {
                    "data": {
                        "text": (
                            f"⚠️ Auto-build stopped after {failures} consecutive failures.\n\n"
                            f"Last error: {last_error}\n\n"
                            f"This task will not be retried automatically. Fix the cause and "
                            f"re-save the Task Status field to trigger a fresh build, or build "
                            f"it manually. If the campaign is already built and launched, move "
                            f"the task out of Ready to Code so it stops being picked up."
                        )
                    }
                },
            )
            _mark_capped_notified(task_gid)
            logger.error(
                f"Task {task_gid} ({task_name_for_guards!r}) hit the retry cap "
                f"({failures} failures) — commented on Asana and giving up"
            )
        else:
            logger.info(f"Task {task_gid} past retry cap ({failures} failures) — skipping")
        return
    ...
    # Any status other than Ready to Code means the task has moved on — a build
    # succeeded (RTC → Ready for QA), or a human took it over. Either way the
    # previous failure streak is stale, so reset it rather than letting it
    # accumulate across unrelated future attempts.
...
async def _dispatch_qa_sms(
    ...
    from utils.sms_grammar import normalize_sms_copy_for_compare as _normalise
    ...
    #
    # `norm_actual` can come back as "" when the built body is nothing but the
    # tracked link — URL-stripping in `_normalise()` removes it entirely. An
    # empty string is trivially "in" any string, so `norm_actual in norm_expected`
    # was silently True whenever the copy was missing altogether, which made this
    # check pass a body that had no copy at all (confirmed on the CZ Best Sellers
    # SMS, 2026-09-08 — Braze had only the link, and QA still reported a match).
    # Guard explicitly: a non-empty expected copy with an empty actual is always
    # a mismatch, never a valid containment case.
    if norm_expected and not norm_actual:
        copy_issues.append(
            f"SMS body has no copy — only the link was found in Braze.\n"
            f"  Expected: {expected_for_compare!r}\n"
            f"  Actual:   {sms_body!r}"
        )
        logger.warning(f"[SMS QA] Copy missing entirely — expected {norm_expected!r}, got empty body")
    elif norm_expected and not (
    ...
    # Character count — count the link at its shortened length, not the raw
    # UTM/Liquid-laden URL, since that's the length Braze actually delivers.
    from utils.sms_grammar import sms_body_length_for_limit_check

    effective_len = sms_body_length_for_limit_check(sms_body)
    if effective_len > 130:
        copy_issues.append(
            f"SMS body is {effective_len} characters after link shortening (limit 130) — shorten copy"
        )

...
def _is_trade_campaign(name: str) -> bool:
    """Trade campaigns are built in the ID Braze workspace regardless of whose
    voice/copy they use (CZ_TRADE, STF_TRADE, TI_TRADE, etc.) — see the
    brand_for_workspace override in build_pt_campaign.py. import_braze.py's
    infer_brand_from_name() tags these YAMLs under the brand in their name
    (e.g. brand: CZ), so a Trade campaign fetched during the CZ/STF/TI/BUR
    loop below needs ID's own credentials, not that loop's brand key."""
    return bool(name) and "TRADE" in name.upper()


def _get_id_braze_credentials():
    """Resolve ID's Braze credentials directly from the environment (not via
    init_config, which mutates import_braze's global CONFIG and would race
    with the ThreadPoolExecutor's concurrent fetch_html calls)."""
    api_key = os.environ.get("BRAZE_API_KEY_ID") or os.environ.get("BRAZE_API_KEY")
    base_url = os.environ.get("BRAZE_BASE_URL_ID") or os.environ.get(
        "BRAZE_BASE_URL", "https://rest.iad-07.braze.com"
    )
...
def fetch_html(campaign_info, html_dir):
    ...
    name = campaign_info["data"].get("name", "")
    ...
    # Trade campaigns live in the ID Braze workspace regardless of which
    # brand they're tagged under — use ID's credentials for this one request.
    api_key = base_url = None
    if _is_trade_campaign(name):
        api_key, base_url = _get_id_braze_credentials()
        if not api_key:
            return None, "Trade campaign requires BRAZE_API_KEY_ID (not configured)"

    # Fetch from Braze
    details = get_campaign_details(braze_id, api_key=api_key, base_url=base_url)

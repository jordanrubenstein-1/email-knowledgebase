...
class SnowflakeClient:
    ...
    def __init__(self, schema=None, database=None):
        ...
        self.private_key_file = os.environ.get("SNOWFLAKE_PRIVATE_KEY_FILE")  # key-pair auth (file path)
        self.private_key_b64 = os.environ.get("SNOWFLAKE_PRIVATE_KEY_B64")  # key-pair auth (inline, base64 of the .p8 PEM)
        self.private_key_passphrase = os.environ.get("SNOWFLAKE_PRIVATE_KEY_PASSPHRASE")  # optional
        ...
        # One auth method must be set: key pair (inline or file), authenticator, or password.
        # Precedence when more than one is present: inline key > key file > authenticator > password.
        if not self.private_key_b64 and not self.private_key_file and not self.authenticator and not self.password:
            required["SNOWFLAKE_PRIVATE_KEY_B64 or SNOWFLAKE_PRIVATE_KEY_FILE or SNOWFLAKE_AUTHENTICATOR or SNOWFLAKE_PASSWORD"] = None
    ...
    def _load_private_key_der(self) -> bytes:
        """Load the private key from an inline base64 env var or a PEM file,
        and return DER-encoded PKCS8 bytes — the format the Snowflake
        connector's `private_key` param expects."""
        if self.private_key_b64:
            try:
                pem_bytes = base64.b64decode(self.private_key_b64)
            except Exception as e:
                print(f"Error: SNOWFLAKE_PRIVATE_KEY_B64 is not valid base64: {e}")
                sys.exit(1)
        else:
            key_path = Path(self.private_key_file).expanduser()
            if not key_path.is_file():
                print(f"Error: SNOWFLAKE_PRIVATE_KEY_FILE not found: {key_path}")
                sys.exit(1)
    ...
    def get_connection(self):
        ...
        try:
            if self._connection is None or self._connection.is_closed():
                ...
                # Auth precedence: key pair (inline or file) > authenticator > password
                if self.private_key_b64 or self.private_key_file:
                    conn_params["private_key"] = self._load_private_key_der()

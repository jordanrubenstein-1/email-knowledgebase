...
def braze_request(endpoint, params=None, method="GET", json_data=None, api_key=None, base_url=None):
    ...
    Args:
        ...
        api_key: Override the global CONFIG's API key (e.g. to reach a
            different brand's Braze workspace for a single request)
        base_url: Override the global CONFIG's base URL to match api_key
...
def get_campaign_details(campaign_id, api_key=None, base_url=None):
    """Fetch detailed info for a specific campaign.

    api_key/base_url let a caller reach a specific brand's Braze workspace
    regardless of the module-level CONFIG — needed for Trade campaigns, whose
    campaign_id only resolves against the ID workspace (see brand_for_workspace
    in build_pt_campaign.py) even when the campaign is tagged under another
    brand by name (e.g. CZ_TRADE).
    """
    data = braze_request(
        "campaigns/details", {"campaign_id": campaign_id}, api_key=api_key, base_url=base_url
    )
    return data

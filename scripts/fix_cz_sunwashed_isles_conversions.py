...
async def _update_conversions_in_place(page: Page) -> None:
    ...
    # non-built-in — see _fill_custom_event_name's docstring in
    # build_pt_campaign.py. Without this, every custom slot's name field was
    # written to whichever slot's field renders last on the page (typically
    # D), not the slot actually being processed — the same root cause behind
    # the "1st and 4th conversion events not selected" bug this script was
    # originally written to patch around by hand.
    custom_rank = 0
